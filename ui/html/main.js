...
// Condition pour vérifier si Tauri est disponible
let event, fs, path, tauri;
if (window.__TAURI__) {
    ({ event, fs, path, tauri } = window.__TAURI__);
}
...
class View {
    ...
    async init() {
        ...
        // Vérifiez si les événements de Tauri sont disponibles avant de les écouter
        if (event) {
    ...
    async update() {
        if (this.is_file_changed && fs && path) { // Vérifiez si fs et path sont disponibles
...
`;
    ...
    async editorSave(editor, e) {
        if (e.ctrlKey && e.keyCode === 19 && this.is_edit_mode && !this.locked && fs && path) { // Vérifiez fs et path
            this.locked = true;
            try {
                let now = Date.now(),
    ...
    appAction(e) {
        if (!event) return;  // Si `event` n'existe pas, retournez
